...
@Component
public class ShopPaymentHandler implements IPipelinePaymentHandler {
    @Override
    public boolean isConditioned(Payment payment) {
        return payment.getTarget()
                .equals(Transaction.Target.SHOP);
    }

    @Override
    public void handle(Payment payment) {
        payment.getSideTransactions()
                .add(new SideTransaction(Transaction.Target.BONUS, Utils.percent(payment.getAmount(), 10), payment));
    }
